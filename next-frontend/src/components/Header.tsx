...
const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Main Header */}
      <header className={`bg-white sticky top-0 z-50 transition-shadow ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              {/* Wishlist */}
              <Link href="/wishlist" className="relative p-2 hover:bg-gray-100 rounded-lg transition" data-testid="wishlist-icon">
              ...
              {/* Cart */}
              <Link href="/cart" className="relative p-2 hover:bg-gray-100 rounded-lg transition" data-testid="cart-icon">
              ...
              {user ? (
                <div className="relative group hidden md:block">
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg hidden group-hover:block">
                    <Link href="/account" className="block px-4 py-2.5 hover:bg-gray-100 text-sm" data-testid="my-account-link">My Account</Link>
                    <Link href="/orders" className="block px-4 py-2.5 hover:bg-gray-100 text-sm" data-testid="my-orders-link">My Orders</Link>
                    {user.is_admin && (
                      <Link href="/admin" className="block px-4 py-2.5 hover:bg-gray-100 text-sm" data-testid="admin-link">Admin Panel</Link>
                    )}
                    <button onClick={logout} className="w-full text-left px-4 py-2.5 hover:bg-gray-100 text-red-600 text-sm" data-testid="logout-button">
                      Logout
                    </button>
                  </div>
                </div>
            ...
            <nav className="container mx-auto px-4 py-4">
              <ul className="space-y-3">
                {user ? (
                  <>
                    <li>
                      <Link href="/account" className="block text-gray-700 font-medium" onClick={() => setMobileMenuOpen(false)}>
                        My Account
                      </Link>
                    </li>
                    <li>
                      <Link href="/orders" className="block text-gray-700 font-medium" onClick={() => setMobileMenuOpen(false)}>
                        My Orders
                      </Link>
                    </li>
                    {user.is_admin && (
                      <li>
                        <Link href="/admin" className="block text-gray-700 font-medium" onClick={() => setMobileMenuOpen(false)}>
                          Admin Panel
                        </Link>
                      </li>
                    )}
